...
PythonとRustで0から45までのフィボナッチ数を計算したときの時間は次のとおりです。
なお、`fibonacci(45) = 1,134,903,170`です。
...
先頭の要素の追加を定数時間にしたい場合は、`collections.deque`（でっく、両端キュー）を使用します。
...
範囲外の位置（インデックス）を指定して要素を取得しようとすると、`IndexError`が発生します。
...
list型には異なる型を格納できますが、その場合、格納する要素には何らかの区分があると考えられます。
...
ファイルレベルで宣言された**グローバル変数**は、そのファイル全体をスコープとするグローバルスコープを持ちます。
関数内で変更不可能な（イミュータブルな）データ型のグローバル変数を利用するためには、そのグローバル変数を利用する前に、`global`キーワードでグローバル変数を利用することを宣言する必要があります。
...
if __name__ == "__main__":
    ...
    # グローバル変数`x`が400ない場合、プログラムがクラッシュ
    assert x == 400
```
...
よって、`a`という**ラベル**で参照されていたメモリ位置のデータ（バイト列）は変更されておらず、int型はイミュータブルです。
これはstr型も同様です。
...
> 例えば、同じ値を持つ変数を2つ宣言したとき、効率性を求めてその変数は同じメモリ位置を示します。
...
ファイルを開くコードを実装した場合、ファイルが存在しない場合など、処理が失敗することがあります。
発生する可能性がある失敗に対して、プログラムがその失敗に対応していない場合、処理が失敗したとき、そのプログラムはクラッシュします。
失敗が発生してもプログラムをクラッシュしないように制御するために、例外処理を使用します。

Pythonでは、`try`文と`except`文を使用して、例外処理を実装します。
`try`ブロックには失敗する可能性のある処理を記述します。
`except`では、その失敗を補足する例外クラスを指定して、そのブロックで補足した例外に対応する処理を記述します。
